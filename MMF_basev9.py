...
class Person:
    def __init__(self, age_, person, popcorns, mms, pc, waters):
        ...
        if self.surcharge != 0:
            print(f"There is a surcharge of {self.surcharge} for your order due to you using a credit card.")
        self.total_price += self.surcharge
        self.profit = (self.ticket_price - TICKET_COST) + (self.snack_price * SNACK_PROFIT_MULTIPLIER)
        print(f"The total price for your order is {self.total_price}")
